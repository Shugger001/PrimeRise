    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3 md:hidden">
        {!orders?.length ? (
          ...
          orders.map((o) => (
            <article key={o.id} className="rounded-2xl border border-admin-border bg-admin-surface p-4">
              <p className="text-xs text-admin-muted">{new Date(o.created_at).toLocaleString()}</p>
              <p className="mt-1 font-medium text-admin-ink">{o.customer_email ?? "—"}</p>
              ...
              <ul className="mt-3 list-inside list-disc space-y-1 text-xs leading-5 text-admin-muted">
